...
public class MessageController {
    ...
    public R deleteMessageRefById(@Valid @RequestBody DeleteMessageRefByIdForm form){
        long rows = messageService.deleteMessageRefId(form.getId());
        return R.ok().put("result",rows == 1 ? true : false );
    }
    ...
    public R refreshMessage(@RequestHeader("token")String token){
        ...
        long unreadCount = messageService.searchUnreadCount(userId);//查询未读数据有多少
        return R.ok().put("lastRows",lastRows).put("unreadRows",unreadCount);
    }
}
